...
export default function DashboardPage() {
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      showToast(
        `Tábor alatti mód ${newValue === "active" ? "bekapcsolva" : "kikapcsolva"}.`
      );
    } catch {
  ...
  if (loading)
    return (
      <div className="py-20 text-center text-gray-400 animate-pulse">Betöltés...</div>
    );
  ...
  const upcomingPrograms = programs
    .filter((p) => new Date(p.startDay) >= today)
    .slice(0, 5);
  const upcomingDeadlines = deadlines
    .filter((d) => new Date(d.date) >= today)
    .slice(0, 5);
  const upcomingOrganizerTasks = organizerTasks
    .filter((t) => new Date(t.day) >= today)
    .slice(0, 5);
      ...
      <div className="space-y-6">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-gray-900 flex items-center gap-2">
              <ClipboardList size={16} className="text-indigo-500" /> Közelgő szervezői
              feladatok
            </h2>
            <AdminButton to="/admin/organizer-tasks" className="text-xs px-3 py-1.5">
              Összes
            </AdminButton>
          </div>
                  ...
                  info={
                    <span className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-[10px] font-bold bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full uppercase">
                        {new Date(t.day).toLocaleDateString("hu-HU")}{" "}
                        {formatOffsetToTime(t.timeOffset)}
                      </span>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <AdminButton to="/admin/programs" className="text-xs px-3 py-1.5">
              Összes
            </AdminButton>
          </div>
          <div className="grid gap-3">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <AdminButton to="/admin/deadlines" className="text-xs px-3 py-1.5">
              Összes
            </AdminButton>
          </div>
          <div className="grid gap-3">
