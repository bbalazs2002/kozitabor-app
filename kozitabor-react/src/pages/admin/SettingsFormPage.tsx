...
const SettingsFormPage = () => {
    ...
    useEffect(() => {
        ...
        if (isNaN(parsedId)) { navigate('/admin/settings', { replace: true }); return; }

        context.getSetting(parsedId).then(data => {
            if (!data) { navigate('/admin/settings', { replace: true }); return; }
            setLabel(data.label);
            setValue(data.value);
    ...
    return (
        <AdminForm title="Beállítás szerkesztése" onSubmit={handleSubmit}>
            <AdminFormCard>
                <div className="mb-2">
                    <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Azonosító</p>
                    <span className="inline-block bg-gray-100 text-gray-600 font-mono text-sm px-3 py-1 rounded-lg">
                        {strId}
                    </span>
                </div>
                <div>
                    <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Beállítás neve</p>
                ...
                {comment && (
                    <div>
                        <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Megjegyzés</p>
