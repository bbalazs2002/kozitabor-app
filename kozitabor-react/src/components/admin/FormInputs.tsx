...
export const Checkbox = ({ label, checked, onChange }: any) => (
  <label className="flex items-center gap-3 cursor-pointer group p-2">
    <div className="relative">
      ...
      ></div>
      <div
        className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${checked ? "translate-x-4" : ""}`}
      ></div>
    </div>
    <span className="text-sm font-semibold text-gray-700 uppercase tracking-wider text-[11px]">
      {label}
    </span>
  </label>
);
...
export const IconPicker = ({ label, value, onChange }: any) => {
  ...
  const iconNames = useMemo(() => {
    return Object.keys(LucideIcons).filter(
      (key) =>
        key !== "createLucideIcon" && typeof (LucideIcons as any)[key] !== "undefined"
    );
  }, []);
            ...
            {filteredIcons.map((name) => {
              ...
              return (
                ...
                >
                  <IconTag
                    size={20}
                    className={`${value === name ? "text-indigo-600" : "text-gray-500 group-hover:text-indigo-600"}`}
                  />
                  <span className="text-[8px] mt-1 text-gray-400 truncate w-full text-center">
                    {name}
                  </span>
                </button>
              );
...
export const NoonDatePicker: React.FC<NoonDatePickerProps> = ({
  value,
  onChange,
  label = "Nap",
}) => {
...
export const TimeOffsetPicker: React.FC<TimeOffsetPickerProps> = ({
  offset,
  onChange,
  label = "Időpont",
}) => {
