...
export const dayOffsetToUtcTs = (day: string | Date, offsetSeconds: number): number => {
  ...
  return (
    Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()) + offsetSeconds * 1000
  );
};
 ...
 */
export const formatUtcOffsetToLocalTime = (
  utcDay: string | Date,
...
export const groupByDay = <T>(items: T[], dateField: keyof T) => {
  return items.reduce(
    (acc, item) => {
      ...
      const d =
        typeof val === "string" || val instanceof Date ? new Date(val as any) : null;
