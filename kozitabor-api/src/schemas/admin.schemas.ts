...
const isoDate = z
  .string()
  .refine((v) => !isNaN(Date.parse(v)), { message: "Érvénytelen dátum formátum." });
...
export const camperTaskSchema = z.object({
  ...
  activityIds: z
    ...
    .min(1, "Legalább egy tevékenység szükséges."),
});
...
export const organizerTaskSchema = z.object({
  ...
  contactIds: z
    .array(z.number().int().positive())
    .min(1, "Legalább egy szervező szükséges."),
  activityIds: z
    .array(z.number().int().positive())
    .min(1, "Legalább egy tevékenység szükséges."),
});
...
export const bulkContactsSchema = z
  .array(
    z.object({
