...
const getCampDate = (dayOffset: number) =>
  new Date(
    Date.UTC(CAMP_START_YEAR, CAMP_START_MONTH, CAMP_START_DAY + dayOffset, 12, 0, 0, 0)
  );
...
async function main() {
  ...
  // 2. ROLES
  const [roleFoszerv, roleLogiszt, roleEgeszseg, roleVezeto, roleSzakacs] =
    await Promise.all([
  ...
  const szabalyzatInfo = await prisma.info.create({
    data: {
      ...
      icon: "FileText",
      content:
        "<h2>Alapszabályok</h2><p>Kérjük, hogy mindenki tartsa be a közös megállapodásokat. A tábor területét csak engedéllyel lehet elhagyni. Éjjeli csend 22:00-tól reggel 07:00-ig.</p>",
    },
  });
  ...
  const helyszinInfo = await prisma.info.create({
    data: {
      ...
      icon: "MapPin",
      content:
        "<p>A tábor a Pilisi erdők szívében található. A legközelebbi buszmegálló 1,5 km-re van. Parkolás a főbejáratnál lehetséges.</p>",
    },
  });
  ...
  const _etrendInfo = await prisma.info.create({
    data: {
      title: "Étkezési rend",
      icon: "UtensilsCrossed",
      content:
        "<ul><li><strong>Reggeli:</strong> 08:00</li><li><strong>Ebéd:</strong> 13:00</li><li><strong>Vacsora:</strong> 19:00</li></ul><p>Ételallergiával rendelkező táborozók jelezzék előre!</p>",
    },
  });
  ...
  const elsosegInfo = await prisma.info.create({
    data: {
      ...
      icon: "HeartPulse",
      content:
        "<p>Az elsősegélyes felszerelés a főépületben, a recepción található. Bármilyen egészségügyi probléma esetén fordulj az egészségügyi felelőshöz.</p>",
    },
  });
  ...
  await prisma.media.createMany({
    data: [
      ...
      {
        url: "https://example.com/hazirend.pdf",
        type: "FILE",
        infoId: szabalyzatInfo.id,
      },
      {
        url: "https://example.com/tabor_helyszin.jpg",
        type: "IMAGE",
        infoId: helyszinInfo.id,
      },
      {
        url: "https://example.com/elsoSeg_utmutato.pdf",
        type: "FILE",
        infoId: elsosegInfo.id,
      },
    ],
  });
  ...
  const programok = [
    ...
    { startOff: "14:00", endOff: "16:00", title: "Kézműves foglalkozás", day: 1 },
    {
      startOff: "17:00",
      endOff: "18:30",
      title: "Sportverseny – csapatok között",
      day: 1,
    },
    ...
    { startOff: "15:00", endOff: "17:00", title: "Tábori olimpia döntők", day: 3 },
    {
      startOff: "20:00",
      endOff: "23:00",
      title: "Gálavacsora és eredményhirdetés",
      day: 3,
    },
    // 4. nap
  ...
  for (const p of programok) {
    await prisma.program.create({
      data: {
        ...
        title: p.title,
        desc:
          p.title.includes("túra") || p.title.includes("kalandpálya")
  ...
  await prisma.setting.createMany({
    data: [
      {
        str_id: "camp_start_date",
        label: "Tábor kezdete",
        value: "2026-07-14",
        comment: "ÉÉÉÉ-HH-NN formátumban",
      },
      {
        str_id: "camp_end_date",
        label: "Tábor vége",
        value: "2026-07-19",
        comment: "ÉÉÉÉ-HH-NN formátumban",
      },
      {
        str_id: "camp_location",
        label: "Táborhely neve",
        value: "Pilisi Erdei Tábor",
        comment: "",
      },
      {
        str_id: "facebook_event_url",
        label: "Facebook esemény linkje",
        value: "",
        comment: "Teljes URL, pl. https://facebook.com/events/...",
      },
      {
        str_id: "group_sheet_url",
        label: "Csoportbeosztás linkje",
        value: "",
        comment: "Teljes URL a csoportbeosztás oldalához",
      },
      {
        str_id: "gyik_url",
        label: "GYIK oldal linkje",
        value: `/kozitabor/info/${gyikInfo.id}`,
        comment: "Az Info bejegyzés relatív útvonala",
      },
      {
        str_id: "app_status",
        label: "Alkalmazás állapota",
        value: "active",
        comment: '"active" vagy "inactive"',
      },
      {
        str_id: "registrant_count",
        label: "Jelentkezők száma",
        value: "0",
        comment: "A táborba jelentkezett táborozók száma",
      },
    ],
  });
