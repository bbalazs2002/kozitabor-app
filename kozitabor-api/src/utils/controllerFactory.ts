...
export const ControllerFactory = {
  ...
  // Egy darab lekérése
  getOne: (service: any, include?: any) => async (req: Request, res: Response) => {
  ...
  // Létrehozás
  create: (service: any) => async (req: Request, res: Response) => {
    const data = await service.create(req.body);
    return res.status(201).json(data);
  },

  // Frissítés
  update: (service: any) => async (req: Request, res: Response) => {
    const data = await service.update(getParsedId(req), req.body);
    return res.json(data);
  },

  // Törlés
  delete: (service: any) => async (req: Request, res: Response) => {
    await service.delete(getParsedId(req));
    return res.json({ success: true, id: getParsedId(req) });
  },

  // Újrarendezés
  reorder: (service: any, orderField?: string) => async (req: Request, res: Response) => {
